...
opt = opts.parse(arg)
print(opt)
local nf_class = 55 --512 --size of class feature
--CLASS_OUTPUT_LAYER_INDEX = 33
...
classifier = torch.load(opt.trained_class_model)
...
function train_batch(dataset)
    ...
    for t = 1,dataset:size(),opt.batch_size do
        ...
        local feval = function(x)
            ...
            model:backward({inputs,class_feats}, df_do)
...
function test_batch(dataset)
    ...
    for t = 1,dataset:size(),opt.batch_size do
        ...
        -- test sample
        local pred = model:forward({inputs,class_feats})
        if opt.use_mask then
		    pred:cmul(masks)
            targets:cmul(masks)
        end
